...
- **Sync-wave hook Jobs.** A `PreSync`/`PostSync` hook Job annotated with `k8s.sustain.io/policy` is a standalone Job as far as k8s-sustain is concerned. The webhook records its recommendation at admission, since Argo CD's hook deletion policy (`argocd.argoproj.io/hook-delete-policy`) can remove the Job before the controller's next reconcile ever sees it. After deletion, the recommendation stays visible on the dashboard as an **inactive** row for the retention window (`--recommendation-retention`, default `72h`) — see [Retention for ephemeral workloads](../concepts/workload-recommendations.md#retention-for-ephemeral-workloads).
