...
3. **GC path (controller).** Cleanup runs through three independent strategies so a `WorkloadRecommendation` cannot outlive its reason for existing:
   ...
   - **Per-cycle sweep.** At the end of every reconcile, the controller lists `WorkloadRecommendation`s carrying the `k8s.sustain.io/policy: <policy>` label and deletes any whose target workload is no longer in the matched set (workload deleted, namespace excluded, annotation removed, kind disabled). Cluster-wide list with no server-side filter would scale O(WLR-count) with cluster size — the label scopes the list server-side and keeps the sweep cheap. Each delete is conditioned on the `uid` **and** `resourceVersion` the sweep observed, because it decides on a copy read from the informer cache: with several reconciles running at once, a workload re-annotated from one policy to another is re-labelled and rewritten by the new policy while the old policy's sweep still holds the pre-rewrite copy. The precondition turns that delete into a conflict — the object is left alone, not counted as deleted, and re-judged on fresh data next cycle — instead of destroying a recommendation that was just computed. A conflict here is an expected outcome, not an error.
   - **Policy-deletion finalizer.** The controller adds the `k8s.sustain.io/cleanup` finalizer to every `Policy`. On `kubectl delete policy`, the finalizer blocks Policy removal until every owned `WorkloadRecommendation` is deleted. This is the only path that GUARANTEES cleanup because it runs synchronously with the user's delete operation — and the preconditions are chosen to keep that promise true. This path deletes on the `uid` **alone**: its rule is "this object belongs to the policy being deleted", which no rewrite can invalidate, so a `resourceVersion` precondition would buy nothing here while letting a WLR rewritten inside the informer cache's propagation window survive. The `uid` still refuses a name that a delete-and-recreate has handed to a different object. Any failure — a conflict included — is returned rather than absorbed, so the finalizer stays on and the deletion is retried; absorbing one would drop the finalizer, let the Policy go, and leave the orphan reaper's next tick as the only remaining collector.
   - **Orphan reaper.** A background goroutine scans every `WorkloadRecommendation` in the cluster on a tick (default 10 min) and deletes any whose `spec.policy` references a Policy that no longer exists. Catches WLRs orphaned by `kubectl delete policy --grace-period=0 --force` (which skips finalizers entirely), controller crashes mid-delete, and Policies renamed before the per-cycle sweep ran. It keeps the strict `uid` + `resourceVersion` preconditions of the sweep: its rule reads `spec.policy`, a field that is rewritten in place when a re-annotated workload's WLR is re-pointed at its new policy, so a stale copy can say "orphan" about an object that has just been adopted and recomputed. Guaranteeing nothing, it can afford the conflict — the object is simply re-judged on the next tick — and the conflict is self-limiting anyway, since the only thing that rewrites a WLR is a reconcile of the policy that owns it.
...
1. **The webhook creates one.** On an admission where no `WorkloadRecommendation` exists for the identity, the webhook creates one with `spec.workloadRef`, `spec.policy`, the `k8s.sustain.io/policy` label, the `k8s.sustain.io/stub: "true"` marker, and an **empty `status`** — then admits the pod unchanged. It also records the admitted pod's per-container requests and limits in `status.observedResources`. The whole write runs on a detached context in a goroutine: an `AdmissionResponse` never blocks on an apiserver write.
   ...
   Those goroutines outlive the admission that started them — up to 30s queued for a write slot, plus 5s for the write itself — so on **shutdown** the webhook drains them before it stops the informer cache they read through: SIGTERM cancels the in-flight stub writes and the process waits (briefly, bounded) for them to unwind, and only then does the cache stop. A request abandoned that way is not lost work; the next admission for the same identity issues it again.
