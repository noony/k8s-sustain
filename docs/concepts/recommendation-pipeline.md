...
1. **Workload-age gate.** If the workload is younger than 10 minutes, it is skipped (controller emits `k8s_sustain_recommendation_skipped_total{reason="workload_too_young"}`). This avoids producing a near-zero percentile that would floor to the hard minimum and trigger a recycle on the next reconcile. The gate is workload-age based rather than sample-count based so it doesn't punish workloads with intrinsically sparse signal (e.g. a daily CronJob). The age is normally the workload object's `CreationTimestamp`, with one refinement for the ephemeral identity kinds, **Job** and **Pod** (bare pods opted in via `k8s.sustain.io/owner-name`): their effective age is the *earliest known* of the object age and the identity's oldest Prometheus sample (`min(min_over_time(timestamp(k8s_sustain:workload_max_pod_cpu:cores{…})[lookback:1m]))`, where the lookback is the CPU window floored at 24h — a lookback capped at the window would cap the observable history age at the window length, so short windows could never satisfy the gate). A standalone Job is re-created on every run, so its object is always seconds old at admission even though usage history has accumulated under the same `(namespace, owner_kind, owner_name)` identity across runs — without the history signal, re-applied Jobs would never pass the gate and never receive an injection or a `WorkloadRecommendation`. A bare pod has no workload object at all (its object age is unknown), so the history age is the gate's only signal: a re-run minutes after a short first run is skipped instead of injected from unstable early samples, while an identity with ≥10 minutes of history injects normally. History can only make a workload look older when an object age exists, and the lookup is fail-open (query failure falls back to object age). The controller-side exception below is unaffected: the controller never gates `Pod`-kind targets, because its gate protects against bad *recycles* and bare pods never recycle. EXCEPTION: a recent OOM in ANY container of the workload (`sum(k8s_sustain:workload_oom_24h) > 0` or a fresh kill from the in-memory watcher) bypasses the gate so a crash-looping container can still get a memory recommendation anchored on the OOM peak — a workload that OOMed anywhere is not "too young to have data". Both the controller (periodic reconcile) and the admission webhook (pod CREATE) apply this gate — without it, the first pod of a brand-new workload would be admitted with the hard-floored minimums (1m CPU / 1Mi memory) and almost certainly crashloop.
2. **Query.** Read the percentile-of-usage from a recording rule over the configured window (`spec.rightSizing.resourcesConfigs.<cpu|memory>.window`). The signal is a genuine per-pod percentile: `quantile_over_time(p, …)` over the `k8s_sustain:workload_max_pod_<cpu|memory>` recording rule, which at each instant is the **busiest replica** (`max by` across pods, per container). Collapsing across pods in the recording rule (rather than at query time) keeps the percentile subquery cheap — one series per workload×container — and immune to pod-name churn, since dead pods drop out of the `max`. Because the percentile already covers the hottest replica, there is **no replica division and no separate per-pod floor**.
